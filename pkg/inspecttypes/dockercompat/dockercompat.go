...
func ContainerFromNative(n *native.Container) (*Container, error) {
	...
	if mountsList := n.Labels[labels.Mounts]; mountsList != "" {
		mounts, err := parseMounts(mountsList)
		if err != nil {
			return nil, err
		}
		c.Mounts = mounts
...
func parseMounts(mountsList string) ([]MountPoint, error) {
	var mounts []MountPoint
	err := json.Unmarshal([]byte(mountsList), &mounts)
	if err != nil {
		return nil, err
	}
