...
func fetchDNSResolverConfig(netOpts options.ContainerNetwork) ([]string, []string, []string, error) {
...
func validateUtsSettings(netOpts options.ContainerNetwork) error {
	...
	if utsNamespace == UtsNamespaceHost && (netOpts.Hostname != "" || netOpts.Domainname != "") {
		return errors.New("conflicting options: cannot set a --hostname and/or --domainname with --uts=host")
	}
