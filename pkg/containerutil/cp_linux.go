...
var (
	...
	ErrShouldNeverHappen      = errors.New("an unexpected error happened - please report this as a bug") // unexpected conditions
...
func CopyFiles(ctx context.Context, client *containerd.Client, container containerd.Container, options types.ContainerCpOptions) (err error) {
	...
	// Actually, this can happen if the container being passed has been deleted since in a racy way
	...
	if sourceSpec.isADir {
		...
		td, err := os.MkdirTemp("", version.RootName+"-cp")
		if err != nil {
			return err
		}
...
func mountSnapshotForContainer(ctx context.Context, client *containerd.Client, conInfo containers.Container, snapshotter string) (string, func() error, error) {
	...
	tempDir, err := os.MkdirTemp("", version.RootName+"-cp")
	if err != nil {
		return "", nil, err
	}
