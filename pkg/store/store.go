...
var (
	// errs.ErrInvalidArgument may be returned by Get, Set, List, or Delete by specific SafeStore implementations
	// (eg: filesystem), when they want to impose implementation dependent restrictions on the identifiers
	// (filesystems typically do).

	// errs.ErrNotFound may be returned by Get or Delete when the requested key is not present in the store

	// errs.ErrSystemFailure may be returned by implementations when an internal failure occurs.
