...
func (cs *CredentialsStore) Retrieve(registryURL *RegistryURL, checkCredStore bool) (*Credentials, error) {
	...
	log.L.Debugf("retrieving credentials from store for %s", registryURL.CanonicalIdentifier())
	...
	log.L.Debugf("all identifiers %v", variants)

	for _, identifier := range variants {
		log.L.Debugf("looking up %s", identifier)
		...
		if returnedCredentials.IdentityToken != "" ||
			...
			log.L.Debugf("found credentials %v", returnedCredentials)
			break
		}
	}
