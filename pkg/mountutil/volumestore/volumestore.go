...
import (
	...
	"github.com/farcloser/lepton/pkg/store"
...
type VolumeStore interface {
	...
	// ListNames returns all existing volumes names without reading them.
	ListNames() ([]string, error)
	// Remove one of more volumes
	Remove(generator func() ([]string, error)) (removed []string, notRemoved []string, warns []error, err error)
	// Prune will call a filtering function expected to return the volumes name to delete
	Prune(filter func(volumes []*native.Volume) ([]string, error)) (removed []string, err error)
...
func (vs *volumeStore) ListNames() (names []string, err error) {
	defer func() {
		if err != nil {
			err = errors.Join(ErrVolumeStore, err)
		}
...
func (vs *volumeStore) Remove(generator func() ([]string, error)) (removed []string, notRemoved []string, warns []error, err error) {
	defer func() {
		if err != nil {
			err = errors.Join(ErrVolumeStore, err)
		}
	...
	err = vs.Locker.WithLock(func() error {
		...
		for _, name := range names {
			...
			if doesExist, err := vs.manager.Exists(name); err != nil {
				...
				notRemoved = append(notRemoved, name)
				warns = append(warns, errors.Join(errs.ErrNotFound, fmt.Errorf("volume %q", name)))
				continue
	...
	return removed, notRemoved, warns, err
}

func (vs *volumeStore) Prune(filter func(vol []*native.Volume) ([]string, error)) (removed []string, err error) {
	defer func() {
		if err != nil {
			err = errors.Join(ErrVolumeStore, err)
		}
