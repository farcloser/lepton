...
import (
	...
	"github.com/containerd/nerdctl/v2/cmd/lepton/builder"
	"github.com/containerd/nerdctl/v2/cmd/lepton/completion"
	"github.com/containerd/nerdctl/v2/cmd/lepton/compose"
	"github.com/containerd/nerdctl/v2/cmd/lepton/container"
	"github.com/containerd/nerdctl/v2/cmd/lepton/helpers"
	"github.com/containerd/nerdctl/v2/cmd/lepton/image"
	"github.com/containerd/nerdctl/v2/cmd/lepton/inspect"
	"github.com/containerd/nerdctl/v2/cmd/lepton/internal"
	"github.com/containerd/nerdctl/v2/cmd/lepton/login"
	"github.com/containerd/nerdctl/v2/cmd/lepton/namespace"
	"github.com/containerd/nerdctl/v2/cmd/lepton/network"
	"github.com/containerd/nerdctl/v2/cmd/lepton/system"
	"github.com/containerd/nerdctl/v2/cmd/lepton/volume"
...
func initRootCmdFlags(rootCmd *cobra.Command, tomlPath string) (*pflag.FlagSet, error) {
	...
	if r, err := os.Open(tomlPath); err == nil {
		...
		if err := dec.Decode(cfg); err != nil {
			return nil, fmt.Errorf("failed to load cli config (not daemon config) from %q (Hint: don't mix up daemon's `config.toml` with `%s.toml`): %w", tomlPath, version.RootName, err)
		}
	...
	rootCmd.PersistentFlags().String("data-root", cfg.DataRoot, fmt.Sprintf("Root directory of persistent %s state (managed by %s, not by containerd)", version.RootName, version.RootName))
