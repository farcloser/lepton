...
func TestConfig(t *testing.T) {
	...
	// Docker does not support loading cli config toml obviously
	...
	testCase.SubTests = []*test.Case{
		...
		},
		{
			...
			Config:      test.WithConfig(nerdtest.CLIToml, `snapshotter = "dummy-snapshotter-via-toml"`),
		},
		{
		...
		},
		{
			...
			Command:     test.Command("info"),
			Expected:    test.Expects(1, []error{errors.New("failed to load cli config")}, nil),
			Config: test.WithConfig(nerdtest.CLIToml, `# containerd config, not cli config
version = 2`),
		},
	}
