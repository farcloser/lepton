		...
		for _, tc := range testCases {
			...
			t.Run("Server "+tc.auth, func(t *testing.T) {
				...
				if tc.auth == "basic" {
					...
					as := testregistry.NewAuthServer(data, helpers, authCa, 0, username, password, false)
		...
		for _, tc := range testCases {
			...
			t.Run(fmt.Sprintf("Login against `tls: %t port: %d auth: %s`", tls, port, auth), func(t *testing.T) {
				...
				if auth == "basic" {
					...
					as := testregistry.NewAuthServer(data, helpers, authCa, 0, username, password, tls)
				...
				// Start the registry with the requested options
				reg := testregistry.NewRegistry(data, helpers, ca, port, authenticator, dependentCleanup)
