...
func TestLoginPersistence(t *testing.T) {
	...
	testCase.Require = nerdtest.IsFlaky(
		"mixed tests using both legacy and non-legacy are considered flaky",
	)
	...
	testCase.Setup = func(data test.Data, helpers test.Helpers) {
		...
		for _, tc := range testCases {
			t.Run("Server "+tc.auth, func(t *testing.T) {
				...
				case "token":
					authCa := testca.New(data, helpers)
					as := testregistry.NewAuthServer(
						data,
						helpers,
						authCa,
						0,
					...
					auth = &testregistry.TokenAuth{
						Address: as.Scheme + "://" + net.JoinHostPort(
							as.IP.String(),
							strconv.Itoa(as.Port),
						),
				...
				reg := testregistry.NewRegistry(data, helpers, nil, 0, auth, dependentCleanup)
							...
							t.Run(
								...
								func(t *testing.T) {
									...
									if !rl.IsLocalhost() || !tls {
										...
										c.Run(base, "http://"+regHost+"/whatever?foo=bar;foo:bar#foo=bar").
											AssertOK()
										c.Run(base, "https://"+regHost+"/whatever?foo=bar&bar=foo;foo=foo+bar:bar#foo=bar").
											AssertOK()
									}
								},
							)
