		...
		Expected: func(data test.Data, helpers test.Helpers) *test.Expected {
			return &test.Expected{
				Output: func(stdout string, info string, t *testing.T) {
					...
					numMatches := 0
					tab := tabutil.NewReader("VOLUME NAME\tDIRECTORY\tSIZE")
					...
					for _, line := range lines {
						...
						assert.Assert(
							...
							size == expectSize,
							fmt.Sprintf(
								"expected size %s for volume %s, got %s",
								expectSize,
								name,
								size,
							)+info,
					...
					assert.Assert(
						t,
						numMatches == len(volSizes),
						fmt.Sprintf("expected %d volumes, got: %d", len(volSizes), numMatches)+info,
					)
				},
			}
		},
...
func TestVolumeLsFilter(t *testing.T) {
	...
	testCase.Setup = func(data test.Data, helpers test.Helpers) {
		vol1, vol2, vol3, vol4 := data.Identifier(
			"1",
		), data.Identifier(
			"2",
		), data.Identifier(
		...
		label1, label2, label3, label4 := "mylabel=label-1", "mylabel=label-2", "mylabel=label-3", "mylabel-group=label-4"
			...
			Expected: func(data test.Data, helpers test.Helpers) *test.Expected {
				return &test.Expected{
					Output: func(stdout string, info string, t *testing.T) {
						...
						assert.Assert(
							t,
							len(volNames) == numMatches,
							fmt.Sprintf("expected %d volumes, got: %d", len(volNames), numMatches),
						)
					},
				}
			},
					...
					Output: func(stdout string, info string, t *testing.T) {
						...
						for _, name := range lines {
							...
							assert.Assert(
								t,
								ok,
								fmt.Sprintf("unexpected volume %s found", name)+info,
							)
						}
					},
				}
