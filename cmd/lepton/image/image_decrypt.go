...
var imageDecryptHelp = fmt.Sprintf(`Decrypt an image locally.
...
Example (encrypt):
  ...
  %s image encrypt --recipient=jwe:mypubkey.pem --platform=linux/amd64,linux/arm64 foo example.com/foo:encrypted
  %s push example.com/foo:encrypted

Example (decrypt):
  %s pull --unpack=false example.com/foo:encrypted
  %s image decrypt --key=mykey.pem example.com/foo:encrypted foo:decrypted
`, version.RootName, version.RootName, version.RootName, version.RootName)
