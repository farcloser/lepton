...
func LoginCommand() *cobra.Command {
	...
	cmd.Flags().StringP(flagUsername, "u", "", "Username")
	cmd.Flags().StringP(flagPassword, "p", "", "Password")
	cmd.Flags().Bool(flagPasswordStdin, false, "Take the password from stdin")

	return cmd
}

func processLoginOptions(cmd *cobra.Command, args []string) (*options.Login, error) {
	...
	passwordStdin, err := cmd.Flags().GetBool(flagPasswordStdin)
	if err != nil {
		return nil, err
	}
