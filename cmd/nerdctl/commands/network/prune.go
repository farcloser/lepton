...
var NetworkDriversToKeep = []string{"host", "none", netutil.DefaultNetworkName}
...
func pruneAction(cmd *cobra.Command, _ []string) error {
	...
	if !force {
		msg := "WARNING! This will remove all custom networks not used by at least one container."
	...
	return network.Prune(ctx, client, cmd.OutOrStdout(), globalOptions, opts)
}
