...
func removeCommand() *cobra.Command {
	...
	cmd.Flags().BoolP(flagCGroup, "c", false, "delete the namespace's cgroup")

	return cmd
}

func removeOptions(cmd *cobra.Command, args []string) (*options.NamespaceRemove, error) {
	cgroup, err := cmd.Flags().GetBool("cgroup")
	if err != nil {
		return nil, err
	}

	return &options.NamespaceRemove{
		NamesList: args,
		CGroup:    cgroup,
	}, nil
}
...
func removeAction(cmd *cobra.Command, args []string) error {
	...
	return namespace.Remove(ctx, client, globalOptions, opts)
