...
func RegistryNames(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	credentialsStore, err := dockerconfigresolver.NewCredentialsStore("")
	if err != nil {
		return nil, cobra.ShellCompDirectiveError
	}

	return credentialsStore.ShellCompletion(), cobra.ShellCompDirectiveNoFileComp
}

func NetworkNamesAll(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	return NetworkNames(cmd, []string{})
}

func NetworkNamesNonSystem(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	return NetworkNames(cmd, []string{"host", "none"})
}
